...
test('the whole product still answers the same number of endpoints', () => {
  ...
  // the prerequisite chain and a student's record. A regression guard on the
  // conversion, not a target.
  const total = PLUGINS.reduce((n, p) => n + p.routes.length, 0)
  assert.equal(total, 129, `expected 129 endpoints across all modules, found ${total}`)
})
